...
exports.createEvent = async (req, res) => {
  ...
  try {
    ...
    if (category_id) {
      const category = await Category.findOne({ where: { id: category_id, workspace_id: req.workspace_id } });
      if (!category) {
    ...
    if (tag_ids && Array.isArray(tag_ids) && tag_ids.length > 0) {
      const tags = await Tag.findAll({ where: { id: tag_ids, workspace_id: req.workspace_id } });
...
exports.getEvents = async (req, res) => {
  try {
    const { start, end, category_id, search } = req.query;
    ...
    // Tarih aralığı filtresi — start_datetime veya end_datetime çakışma kontrolü
    if (start && end) {
      whereClause = {
        [Op.and]: [
          whereClause,
          {
            [Op.or]: [
              { start_datetime: { [Op.between]: [new Date(start), new Date(end)] } },
              { end_datetime: { [Op.between]: [new Date(start), new Date(end)] } },
              {
                [Op.and]: [
                  { start_datetime: { [Op.lte]: new Date(start) } },
                  { end_datetime: { [Op.gte]: new Date(end) } }
                ]
              }
            ]
          }
...
exports.updateEvent = async (req, res) => {
  ...
  try {
    ...
    let event = await Event.findOne({ where: { id: req.params.id, workspace_id: req.workspace_id } });
...
exports.deleteEvent = async (req, res) => {
  try {
    const event = await Event.findOne({ where: { id: req.params.id, workspace_id: req.workspace_id } });
...
// POST /api/events/:id/duplicate — Etkinliği kopyala
exports.duplicateEvent = async (req, res) => {
  try {
    const { Event, Category, Tag } = require('../models');
    const original = await Event.findOne({ where: { id: req.params.id, workspace_id: req.workspace_id } });
    if (!original) return res.status(404).json({ success: false, message: 'Etkinlik bulunamadı' });

    const { start_datetime, end_datetime } = req.body;
    const origStart = new Date(original.start_datetime);
    const origEnd = new Date(original.end_datetime);
    const duration = origEnd - origStart;

    const newStart = start_datetime ? new Date(start_datetime) : new Date(origStart.getTime() + 86400000);
    const newEnd = end_datetime ? new Date(end_datetime) : new Date(newStart.getTime() + duration);
    ...
    const created = await Event.findByPk(copy.id, {
      include: [{ model: Category, attributes: ['id', 'name', 'color_hex'] }]
    });
    res.status(201).json({ success: true, data: created });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Etkinlik kopyalanamadı', error: error.message });
  }
};
